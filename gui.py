...
class Player(Tk.Frame):
    ...
    def __init__(self, parent, title=None):
        ...
        #This panel holds the game information
      
        
        ctrlpanel3 = ttk.Frame(self.parent)
        self.quarter = Tk.IntVar()
        quarterSelect = Tk.LabelFrame(ctrlpanel3, text="Quarter", padx=5, pady=5)
        r1 = Tk.Radiobutton(quarterSelect, text="1", variable=self.quarter, value=1)
        r2 = Tk.Radiobutton(quarterSelect, text="2", variable=self.quarter, value=2)
        r3 = Tk.Radiobutton(quarterSelect, text="3", variable=self.quarter, value=3)
        r4 = Tk.Radiobutton(quarterSelect, text="4", variable=self.quarter, value=4)
        r1.pack(side=Tk.LEFT)
        r2.pack(side=Tk.LEFT)
        r3.pack(side=Tk.LEFT)
        r4.pack(side=Tk.LEFT)
        quarterSelect.pack(side=Tk.LEFT, fill=Tk.X)
        self.gameID = Tk.StringVar()
        e = Tk.Entry(ctrlpanel3, textvariable=self.gameID)
        e.pack(side=Tk.RIGHT)
        gameIDLabel = Tk.Label(ctrlpanel3, text="Game ID")
        gameIDLabel.pack(side=Tk.LEFT)

        self.fileName = Tk.StringVar()
        self.fileName.set("0");
        self.fileNameEntry = Tk.Entry(ctrlpanel3, textvariable=self.fileName)
        self.fileNameEntry.insert(0, str(self.fileName.get()))
        self.fileNameEntry.pack(side=Tk.LEFT)

        fileNameLabel = Tk.Label(ctrlpanel3, text="File Name")
        ...
        startSlice = ttk.Button(ctrlpanel, text="Start Slice", command=self.startSlice)
        endSlice = ttk.Button(ctrlpanel, text="End Slice", command=self.endSlice)

        pause.pack(side=Tk.LEFT)
        play.pack(side=Tk.LEFT)
        stop.pack(side=Tk.LEFT)
        volume.pack(side=Tk.LEFT)
        startSlice.pack(side=Tk.LEFT)
        endSlice.pack(side=Tk.LEFT)
    ...
    def startSlice(self):
        self.timeStart = self.player.get_time() / 1000 #VLC returns time in ms
    ...
    def endSlice(self):
        ...
        print(time)
        self.duration = time - self.timeStart
        #php stuff

        print("123.avi", self.timeStart, round(self.duration, 3), 2017, self.gameID.get(), self.quarter.get(), str(self.fileName.get()).zfill(3))
        tempencode.sendGETRequest(tempencode.encodeSliceURL("123.avi", self.timeStart, round(self.duration, 3), 2016, self.gameID.get(), self.quarter.get(), str(self.fileName.get()).zfill(3)))
        
        if(RepresentsInt(self.fileName.get())):
            next = int(self.fileName.get()) + 1
            print(next)
            self.fileNameEntry.delete(0, 'end')
            self.fileNameEntry.insert(0, str(next))
        #self.fileName.set((int(self.fileName.get()) + 1))
...
if __name__ == "__main__":
    ...
    player = Player(root, title="eStadium Video Slicer")
